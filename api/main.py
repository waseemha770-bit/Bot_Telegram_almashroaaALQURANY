...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'لوحة الإدارة' in text and adm:
        ...
        if await has_perm(user_id, "stats"): 
            ...
            btns.append([InlineKeyboardButton("📊 | الرسوم البيانية", callback_data="admin_charts")])
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ================= 🌟 الاستيراد والتصدير 🌟 =================
    if data == "import_confirm" and await has_perm(user_id, "upload"):
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "WAIT_EXCEL"}}, upsert=True)
        msg = "📥 <b>الاستيراد الذكي (Excel)</b>\n\nأرسل ملف الإكسل الآن (`.xlsx`).\nسيقوم البوت بالإضافة والتحديث بذكاء."
    ...
    if data == "admin_charts" and await has_perm(user_id, "stats"):
        await safe_edit(query, "⏳ جاري توليد الرسوم البيانية الذكية...")
        try:
            total_users = await db.users.count_documents({})
            active_users = await db.users.count_documents({"last_active": {"$gte": time.time() - 7*86400}})
            ...
            u_conf = {
                "type": "doughnut",
                "data": {
                    "labels": ["نشط (آخر 7 أيام)", "غير نشط"],
                    "datasets": [{"data": [active_users, inactive_users], "backgroundColor": ["#2ecc71", "#e74c3c"]}]
                },
                "options": {"title": {"display": True, "text": "نشاط المستخدمين", "fontSize": 18}}
            }
            u_url = f"https://quickchart.io/chart?c={urllib.parse.quote(json.dumps(u_conf))}"
            
            top_lessons = await db.lesson_stats.find().sort("views", -1).limit(5).to_list(length=None)
            labels = [l["lesson"][:20] for l in top_lessons] if top_lessons else ["لا يوجد"]
            views = [l.get("views", 0) for l in top_lessons] if top_lessons else [0]
            
            l_conf = {
                "type": "bar",
                "data": {
                    "labels": labels,
                    "datasets": [{"label": "المشاهدات", "data": views, "backgroundColor": "#3498db"}]
                },
                "options": {
                    "title": {"display": True, "text": "أعلى 5 دروس تفاعلا", "fontSize": 18},
                    "legend": {"display": False},
                    "scales": {"yAxes": [{"ticks": {"beginAtZero": True, "stepSize": 1}}]}
                }
            }
            l_url = f"https://quickchart.io/chart?c={urllib.parse.quote(json.dumps(l_conf))}"
            
            await context.bot.send_photo(chat_id=chat_id, photo=u_url)
            await context.bot.send_photo(chat_id=chat_id, photo=l_url, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_menu")]]))
            await query.message.delete()
            
        except Exception as e:
            await safe_edit(query, f"❌ حدث خطأ أثناء التوليد: {e}", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_menu")]]))
        return
