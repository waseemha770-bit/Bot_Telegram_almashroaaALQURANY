...
# ==========================================
# 1. تهيئة الاتصال بـ MongoDB (النسخة السريعة اللامتزامنة Motor)
# ==========================================
...
if MONGODB_URI:
    try:
        # استخدام Motor يمنع تجميد البوت ويسمح بآلاف الطلبات المتزامنة
...
# ==========================================
# 2. القوائم ونظام الحماية (Anti-Spam)
# ==========================================
user_last_action = {}  # ذاكرة مؤقتة لمنع السبام
...
async def check_spam(user_id: str) -> bool:
    """يمنع المستخدم من إرسال أكثر من طلب في أقل من ثانية لحماية السيرفر"""
    now = time.time()
...
# ==========================================
# 3. دوال التعامل مع MongoDB (اللامتزامنة Async)
# ==========================================
...
async def add_media(category, lesson, media_type, file_id):
    if db is not None:
...
async def submit_answer_to_db(user_id, q_id, is_correct, points, category):
    if db is not None:
        user = await db.users.find_one({"_id": str(user_id)})
        if user:
...
async def handle_media_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kb == USER_KB: return # منع المستخدمين العاديين من الرفع
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_spam(user_id): return # حماية من السبام
    ...
    if state == "WAIT_LES":
        await add_media(temp_data["category"], text, temp_data["type"], temp_data["file_id"])
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_spam(user_id):
        return await query.answer("الرجاء التمهل! ✋", show_alert=False)
    ...
    if data.startswith("delles_"):
        ...
        if lesson_data and "title" in lesson_data:
             ...
             if db is not None:
                 await db.library.delete_many({"lesson": title})
    ...
    if data.startswith("send_"):
        ...
        try:
            ...
            await context.bot.send_message(chat_id, "⚠️ الملف غير متاح حالياً أو تم حذفه من السيرفر.")
        return
    ...
    if data.startswith("ans_"):
        ...
        # حماية من التلاعب بالتوقيت والثغرات
        ...
        await submit_answer_to_db(user_id, q_id, is_correct, pts, cat)
...
async def send_question(context, chat_id, category, user_id=None, msg_id=None):
    if db is None: return await context.bot.send_message(chat_id, "⚠️ خطأ في الاتصال بقاعدة البيانات.")
