...
# ==========================================
# دوال استخراج الروابط الذكية (الجديدة)
# ==========================================
async def get_or_create_link(msg, context, chat_id):
    """تستخرج الرابط مباشرة من المحتوى المحول دون نشره، أو تنسخه للقناة إذا كان رفعاً مباشراً"""
    if msg.forward_from_chat and str(msg.forward_from_chat.type) == "channel":
        if msg.forward_from_chat.username: return f"https://t.me/{msg.forward_from_chat.username}/{msg.forward_from_message_id}"
        else: return f"https://t.me/c/{str(msg.forward_from_chat.id).replace('-100', '')}/{msg.forward_from_message_id}"
    
    text = msg.text or msg.caption
    if text and text.startswith("http"): return text.strip()
        
    if CHANNEL_ID:
        try:
            chat_target = CHANNEL_ID if CHANNEL_ID.startswith('@') or CHANNEL_ID.startswith('-100') else f"@{CHANNEL_ID}"
            res = await context.bot.copy_message(chat_id=chat_target, from_chat_id=chat_id, message_id=msg.message_id)
            ch_name = CHANNEL_ID.replace('@', '').replace('https://t.me/', '')
            return f"https://t.me/{ch_name}/{res.message_id}"
        except Exception: return None
    return None

async def process_generic_upload(user_id, chat_id, final_link, msg, context):
    """توحيد عملية ربط المحتوى العام بالسلاسل"""
    if not final_link: 
        return await msg.reply_text("❌ تعذر حفظ المحتوى. تأكد أن البوت مشرف في القناة الافتراضية.")
    ...
    sent_msg = await msg.reply_text("📥 <b>تم استلام المحتوى!</b>\nاختر السلسلة لربطه بها:", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(btns))
    await db.users.update_one({"_id": user_id}, {"$set": {"state": "UPLOADING", "temp_data": {"file_id": final_link}, "last_msg_id": sent_msg.message_id}}, upsert=True)
...
async def get_type_keyboard():
    ...
    kb.append([InlineKeyboardButton("➕ إنشاء زر مخصص لهذا المحتوى", callback_data="utype_custom_new")])
...
async def show_lesson_ui(context, chat_id, doc_id, message_id=None, user_id=None):
    ...
    for item in items:
        ...
        if safe_link:
            if f_type.startswith("custom_"):
                btn_text = item.get("custom_name", "🔗 محتوى إضافي")
            else:
                t_info = types_dict.get(f_type, {"name": "محتوى", "icon": "📁"})
                btn_text = f"{t_info['icon']} {t_info['name']}"
            
            row.append(InlineKeyboardButton(btn_text, url=safe_link))
...
# ==========================================
# معالجة الوسائط المتعددة وملفات الإكسل
# ==========================================
async def handle_media_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الاستيراد المباشر למحتوى الدرس 
    if state == "WAIT_CONTENT":
        if not await has_perm(user_id, "upload"): return
        await clean_chat_history(user_id, chat_id, context)
        link = await get_or_create_link(msg, context, chat_id)
        if link:
            ...
            await msg.reply_text("❌ تعذر حفظ المحتوى. تأكد أن البوت مشرف في القناة.")
        return

    # استيراد إكسل الأسئلة
                    ...
                    if les_col and cat_col and pd.notna(row.get(les_col)) and pd.notna(row.get(cat_col)):
                        ...
                        for t_name, t_id in name_to_id.items():
                            ...
                            if "فيديو" in excel_type_val: t_val = "type_video"
                                ...
                                t_val = f"custom_{int(time.time()*1000)}_{random.randint(100,999)}"
                                custom_name = excel_type_val
                        ...
                        if l_val == "حذف":
                            ...
                            update_data = {"file_id": l_val, "updated_at": time.time()}
                            if custom_name: update_data["custom_name"] = custom_name
                            await db.library.update_one({"category": cat_val, "lesson": les_val, "type": t_val}, {"$set": update_data}, upsert=True)
                            count_add += 1
    ...
    if not state:
        if await has_perm(user_id, "upload"):
            link = await get_or_create_link(msg, context, chat_id)
            if link: await process_generic_upload(user_id, chat_id, link, msg, context)
        return
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db is None: return await msg.reply_text("⚠️ خطأ في الاتصال بقاعدة البيانات.")
    ...
    if text in ['إلغاء', '❌ إلغاء', '/cancel']:
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "", "temp_data": {}}}, upsert=True)
        await clean_chat_history(user_id, chat_id, context)
        sent_msg = await msg.reply_text("✅ <b>تم إلغاء العملية.</b>", parse_mode="HTML", reply_markup=kb)
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return

    if text.startswith('/start'):
        u_name = msg.from_user.first_name
        u_username = msg.from_user.username
    ...
    # 🌟 إنشاء زر مخصص 🌟
    if state == "WAIT_CUSTOM_BTN_NAME":
        if not await has_perm(user_id, "upload"): return
        custom_name = text.strip()
        link = temp_data.get("pending_link") or temp_data.get("file_id")
        cat = temp_data.get("mgr_target_cat") or temp_data.get("category", "عام")
        les = temp_data.get("mgr_target_les") or temp_data.get("lesson", "عام")
        
        custom_type_id = f"custom_{int(time.time() * 1000)}_{random.randint(10,99)}"
        
        await db.library.update_one(
            {"category": cat, "lesson": les, "type": custom_type_id}, 
            {"$set": {"file_id": link, "custom_name": custom_name, "created_at": time.time()}}, 
            upsert=True
        )
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "", "temp_data": {}}}, upsert=True)
        await clean_chat_history(user_id, chat_id, context)
        
        les_oid = temp_data.get("mgr_target_les_id")
        btns = []
        if les_oid: btns.append([InlineKeyboardButton("🔙 | العودة للدرس", callback_data=f"mgr_les_{les_oid}")])
        else: btns.append([InlineKeyboardButton("🔙 | العودة للوحة الإدارة", callback_data="admin_menu")])
        
        sent_msg = await msg.reply_text(f"🎉 <b>تم إنشاء الزر وربط المحتوى بنجاح!</b>\nالزر: {html.escape(custom_name)}\nالدرس: {html.escape(les)}", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(btns))
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return

    if state == "WAIT_COMP_TIME":
        if not await has_perm(user_id, "publish"): return
        if not text.isdigit(): return await msg.reply_text("⚠️ يرجى إرسال رقم (عدد الدقائق) فقط.")
        minutes = int(text)
        await db.users.update_one({"_id": user_id}, {"$set": {"state": ""}})
        await clean_chat_history(user_id, chat_id, context)
        
        q_oid = get_safe_oid(temp_data.get("comp_q_id"))
        if not q_oid: return await msg.reply_text("السؤال غير متوفر.")
        q_doc = await db.questions.find_one({"_id": q_oid})
        if not q_doc: return await msg.reply_text("السؤال غير متوفر.")
        ...
        try:
            ...
            sent_msg = await msg.reply_text(f"🎉 <b>تم إرسال المسابقة للقناة بنجاح!</b>\nسيتم إغلاقها تلقائياً بعد {minutes} دقيقة.", parse_mode="HTML", reply_markup=kb)
            await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        except Exception as e:
            sent_msg = await msg.reply_text(f"❌ حدث خطأ في النشر للقناة: <code>{e}</code>", parse_mode="HTML")
            await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_MGR_NEW_CAT":
        ...
        sent_msg = await msg.reply_text(f"✅ تم إضافة السلسلة الجديدة: (<b>{html.escape(new_cat)}</b>) بنجاح!", reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_MGR_NEW_LES":
        ...
        sent_msg = await msg.reply_text(f"✅ تم إنشاء الدرس الجديد بنجاح!", reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_MGR_EDIT_LES":
        ...
        await clean_chat_history(user_id, chat_id, context)
        btns = [[InlineKeyboardButton("🔙 | العودة للسلسلة", callback_data=f"mgr_cat_view_{cat_id}")]]
        sent_msg = await msg.reply_text(f"✅ تمت العملية وتحديث الروابط والأسئلة المرتبطة به.", reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_TYPE_DATA" and await has_perm(user_id, "publish"):
        ...
        sent_msg = await msg.reply_text(f"✅ تم إضافة النوع ({icon} {html.escape(name)}) بنجاح!", reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_EDIT_TYPE" and await has_perm(user_id, "publish"):
        ...
        await clean_chat_history(user_id, chat_id, context)
        btns = [[InlineKeyboardButton("🔙 | العودة", callback_data="admin_content_types")]]
        sent_msg = await msg.reply_text(f"✅ تم تحديث النوع بنجاح!", reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return

    if state == "WAIT_CHAN_ID" and await has_perm(user_id, "publish"):
        ch = text.strip()
        if not ch.startswith('@') and not ch.startswith('-100'): return await msg.reply_text("⚠️ معرّف القناة يجب أن يبدأ بـ @ أو -100")
        ...
        sent_msg = await msg.reply_text(f"✅ تم إضافة القناة بنجاح!", reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_TPL_NAME":
        ...
        await clean_chat_history(user_id, chat_id, context)
        reply_m = f"""✅ تم اختيار اسم القالب: <b>{html.escape(text)}</b>\n\n✍️ أرسل الآن محتوى القالب وتصميمه.\nاستخدم المتغيرات بالأقواس المعكوفة:\n<code>{{سلسلة}}</code> ، <code>{{درس}}</code> ، <code>{{تاريخ}}</code> ، <code>{{تذييل}}</code>"""
        sent_msg = await msg.reply_text(reply_m, parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء", callback_data="admin_cancel")]]))
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_TPL_CONTENT":
        ...
        sent_msg = await msg.reply_text(f"🎉 <b>تم حفظ القالب بنجاح!</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(btns))
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_FOOTER_TEXT":
        ...
        await db.users.update_one({"_id": user_id}, {"$set": {"state": ""}}, upsert=True)
        await clean_chat_history(user_id, chat_id, context)
        sent_msg = await msg.reply_text("✅ <b>تم حفظ التذييل بنجاح!</b>", parse_mode="HTML", reply_markup=kb)
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return

    if state == "WAIT_ADMIN_ID" and await has_perm(user_id, "manage_admins"):
        new_admin = text.strip()
        if not new_admin.isdigit(): return await msg.reply_text("⚠️ الآيدي يجب أن يكون أرقاماً فقط.")
    ...
    if state == "WAIT_Q_CORRECT":
        ...
        await clean_chat_history(user_id, chat_id, context)
        sent_msg = await msg.reply_text("❌ أرسل الآن <b>الإجابات الخاطئة</b> مفصولة بفاصلة:", parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء", callback_data="admin_cancel")]]))
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return
    ...
    if state == "WAIT_Q_WRONG":
        ...
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "", "temp_data": {}}}, upsert=True)
        await clean_chat_history(user_id, chat_id, context)
        sent_msg = await msg.reply_text("🎉 <b>تم حفظ السؤال بنجاح!</b>", parse_mode="HTML", reply_markup=kb)
        await db.users.update_one({"_id": user_id}, {"$set": {"last_msg_id": sent_msg.message_id}}, upsert=True)
        return

    if not state:
        if msg.forward_from_chat or text.startswith("http"):
            if await has_perm(user_id, "upload"):
                link = await get_or_create_link(msg, context, chat_id)
                await process_generic_upload(user_id, chat_id, link, msg, context)
            return

    try: await msg.reply_text("الرجاء استخدام الأزرار المتاحة 👇", reply_markup=kb)
    except: pass
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "export_db" and await has_perm(user_id, "stats"):
        ...
        try:
            ...
            types_docs = await db.content_types.find({}).to_list(length=None)
            types_dict = {t["_id"]: t["name"] for t in types_docs}
            
            lib_list = []
            for item in lib_data:
                t_id = str(item.get("type", ""))
                if t_id.startswith("custom_"): t_name = item.get("custom_name", "مخصص")
                else: t_name = types_dict.get(t_id, "محتوى")
                lib_list.append({
                    "السلسلة": item.get("category", ""),
                    "المحاضرة /الدرس": item.get("lesson", ""),
                    "النوع": t_name,
                    "الرابط": item.get("file_id", "")
                })
            ...
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                if not df_lib.empty: df_lib.to_excel(writer, sheet_name='المشروع القرأني', index=False)
                if q_list: pd.DataFrame(q_list).to_excel(writer, sheet_name='قيم_نفسك', index=False)
    ...
    if data.startswith("ask_del_link_"):
        oid = get_safe_oid(data.replace("ask_del_link_", ""))
        if not oid: return await query.answer("عنصر غير موجود.", show_alert=True)
        btns = [[InlineKeyboardButton("✅ نعم، احذف المحتوى", callback_data=f"del_link_{str(oid)}")], [InlineKeyboardButton("❌ تراجع", callback_data="mgr_manage_links")]]
        await safe_edit(query, "⚠️ هل أنت متأكد من حذف هذا المحتوى (الزر) من الدرس؟", InlineKeyboardMarkup(btns))
        return
    ...
    if data.startswith("mgr_les_"):
        ...
        btns = [
            [InlineKeyboardButton("🔗 | إرفاق محتوى جديد بالدرس", callback_data="mgr_attach_content")],
            [InlineKeyboardButton("🎛️ | إدارة أزرار ومحتويات الدرس", callback_data="mgr_manage_links")],
    ...
    if data == "mgr_manage_links":
        ...
        for item in items:
            ...
            if safe_link:
                f_type = str(item.get("type", ""))
                if f_type.startswith("custom_"): btn_text = item.get("custom_name", "🔗 محتوى")
                else:
                    t_info = types_dict.get(f_type, {"name": "محتوى", "icon": "📁"})
                    btn_text = f"{t_info['icon']} {t_info['name']}"
                btns.append([InlineKeyboardButton(f"🗑️ حذف ({btn_text})", callback_data=f"ask_del_link_{str(item['_id'])}")])
                
        les_oid = user.get("temp_data", {}).get("mgr_target_les_id")
        btns.append([InlineKeyboardButton("🔙 | رجوع للدرس", callback_data=f"mgr_les_{les_oid}")])
        
        if len(btns) == 1: await safe_edit(query, f"📖 الدرس: <b>{html.escape(les)}</b>\n\nلا يوجد أزرار أو محتويات مضافة حالياً.", InlineKeyboardMarkup(btns))
        else: await safe_edit(query, f"📖 الدرس: <b>{html.escape(les)}</b>\nاختر المحتوى الذي تريد حذفه من هذا الدرس:", InlineKeyboardMarkup(btns))
        return

    if data.startswith("del_link_"):
        oid = get_safe_oid(data.replace("del_link_", ""))
        if oid: await db.library.delete_one({"_id": oid})
        les_oid = user.get("temp_data", {}).get("mgr_target_les_id")
        await safe_edit(query, "✅ تم حذف الزر والمحتوى بنجاح!", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 | رجوع لإدارة الدرس", callback_data=f"mgr_les_{les_oid}")]]) )
        return
    ...
    if data == "admin_content_types" and await has_perm(user_id, "publish"):
        ...
        btns.append([InlineKeyboardButton("➕ | إضافة نوع جديد (كقالب عام)", callback_data="add_type")])
        btns.append([InlineKeyboardButton("🔙 | رجوع للوحة", callback_data="admin_menu")])
        await safe_edit(query, "🎛️ <b>إدارة أنواع المحتوى (الأزرار العامة):</b>\nتستخدم كقوالب سريعة عند إضافة الروابط:", InlineKeyboardMarkup(btns))
        return
    ...
    if data.startswith("pubmed_"):
        ...
        for item in items:
            ...
            if safe_link:
                if f_type.startswith("custom_"):
                    dynamic_links[item.get("custom_name", "رابط")] = safe_link
    ...
    if data.startswith("pconf_"):
        ...
        if draft_format == "btns":
            ...
            for item in items:
                ...
                if safe_link:
                    f_type = str(item.get("type", ""))
                    if f_type.startswith("custom_"): t_name = item.get("custom_name", "🔗 رابط")
                    else: t_name = next((t["name"] for t in types_docs if t["_id"] == f_type), "رابط")
...
👇 <b>أرسل ملف الإكسل الآن كـ (مستند / Document):</b>"""
    ...
    if data == "utype_custom_new":
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "WAIT_CUSTOM_BTN_NAME"}}, upsert=True)
        await safe_edit(query, "✍️ أرسل <b>اسم الزر</b> الذي سيظهر للطلاب لهذا المحتوى (مع إيموجي):\nمثال: <code>📝 ملزمة إضافية</code>", InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء", callback_data="admin_cancel")]]))
        return
    ...
    if data.startswith("utype_"):
        ...
        if user.get("state") == "WAIT_CONTENT_TYPE":
            link = temp_data.get("pending_link")
            cat, les = temp_data.get("mgr_target_cat"), temp_data.get("mgr_target_les")
            await db.library.update_one({"category": cat, "lesson": les, "type": val}, {"$set": {"file_id": link, "updated_at": time.time()}}, upsert=True)
            await db.users.update_one({"_id": user_id}, {"$set": {"state": "", "temp_data": {}}}, upsert=True)
            await safe_edit(query, f"🎉 <b>تم ربط المحتوى بالدرس بنجاح!</b>\nالدرس: {html.escape(les)}", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 | العودة للدرس", callback_data=f"mgr_les_{temp_data.get('mgr_target_les_id')}")]]) )
            return
            
        cat, les, f_link = temp_data.get("category", "عام"), temp_data.get("lesson", "عام"), temp_data.get("file_id")
    ...
    if data.startswith("ul_"):
        ...
        if doc_id == "new":
            await db.users.update_one({"_id": user_id}, {"$set": {"state": "WAIT_UPL_LES_TEXT"}}, upsert=True)
        ...
        oid = get_safe_oid(doc_id)
        if not oid: 
            try: await query.answer("⚠️ القائمة قديمة، يرجى تحديث النظام بإرسال /start.", show_alert=True)
            except: pass
            return
        doc = await db.library.find_one({"_id": oid})
        if not doc: return await safe_edit(query, "⚠️ عذراً، لم يعد هذا العنصر متوفراً.")
    ...
    if data.startswith("uc_"):
        ...
        if doc_id == "new":
            await db.users.update_one({"_id": user_id}, {"$set": {"state": "WAIT_UPL_CAT_TEXT"}}, upsert=True)
        ...
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "UPLOADING", "temp_data": temp_data}}, upsert=True)
        
        pipeline = [{"$match": {"category": cat_name}}, {"$group": {"_id": "$lesson", "doc_id": {"$first": "$_id"}}}, {"$sort": {"doc_id": 1}}]
        lessons = await db.library.aggregate(pipeline).to_list(length=None)
        
        btns = [[InlineKeyboardButton(f"📖 | {idx}- {les['_id']}", callback_data=f"ul_{str(les['doc_id'])}")] for idx, les in enumerate(lessons[:90], 1)]
