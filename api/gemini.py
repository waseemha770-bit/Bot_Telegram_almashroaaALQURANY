...
async def handle_media_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "WAIT_Q_EXCEL" and msg.document:
        if not msg.document.file_name.endswith(('.xlsx', '.xls')): return await msg.reply_text("⚠️ يرجى رفع ملف الاختبار بصيغة Excel (.xlsx) فقط.")
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                q_col, ans_col = next((c for c in cols if 'سؤال' in c), None), next((c for c in cols if 'صحيح' in c), None)
        ...
        try:
            ...
            if df_q is not None:
                ...
                for _, row in df_q.iterrows():
                    q_col, ans_col = next((c for c in df_q.columns if 'السؤال' in str(c)), None), next((c for c in df_q.columns if 'الصحيح' in str(c)), None)
                    cat_col, les_col = next((c for c in df_q.columns if 'السلسلة' in str(c)), None), next((c for c in df_q.columns if 'الدرس' in str(c) or 'المحاضرة' in str(c)), None)
...
# ==========================================
# معالجة الأزرار بجميع أشكالها (Stateless)
# ==========================================
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ================= 🌟 الأزرار الأساسية (إلغاء والعودة للرئيسية) 🌟 =================
    ...
    # ================= 🌟 أزرار الإحصائيات وإدارة المشرفين 🌟 =================
    if data == "admin_stats" and await has_perm(user_id, "stats"):
        ...
        try:
            ...
            await context.bot.send_document(chat_id=chat_id, document=output, filename="قاعدة_بيانات_البوت.xlsx", caption="📊 <b>تقرير الإحصائيات</b>", parse_mode="HTML")
    ...
    if data == "admin_manage" and user_id == OWNER_ID:
        ...
        await safe_edit(query, "👥 <b>إدارة المشرفين:</b>\nانقر للتعديل:", InlineKeyboardMarkup(btns))
        return
    ...
    # ================= 🌟 رسائل التأكيد والتحذير الحساسة 🌟 =================
    ...
    # ================= 🌟 إدارة أنواع المحتوى والقنوات والقوالب 🌟 =================
    ...
    # ================= 🌟 قوائم وأقسام واجهة المستخدم والتنقل 🌟 =================
