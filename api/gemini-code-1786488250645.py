...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "comp_leaderboard":
        ...
        if not top_users: 
            try: await query.answer("لا يوجد متسابقين بنقاط حتى الآن.", show_alert=True)
            except: pass
            return
    ...
    if data.startswith("comp_les_"):
        ...
        if not all_qs: 
            try: await query.answer("⚠️ لا توجد أسئلة مضافة لهذا الدرس!", show_alert=True)
            except: pass
            return
    ...
    if data.startswith("editadm_") and await has_perm(user_id, "manage_admins"):
        ...
        if not adm_doc: 
            try: await query.answer("لم يتم العثور", show_alert=True)
            except: pass
            return
    ...
    if data.startswith("mgr_add_les_"):
        ...
        if not doc: 
            try: await query.answer("عنصر غير موجود.", show_alert=True)
            except: pass
            return
    ...
    if data.startswith("mgr_les_"):
        ...
        doc = await db.library.find_one({"_id": oid})
        if not doc: 
            try: await query.answer("الدرس غير موجود", show_alert=True)
            except: pass
            return
    ...
    if data.startswith("pubmed_"):
        ...
        if fmt_type == "btns":
            ...
            if not tpl_doc: 
                try: await query.answer("القالب غير موجود!", show_alert=True)
                except: pass
                return
