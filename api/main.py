...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "WAIT_POLL_Q" and await has_perm(user_id, "publish"):
        temp_data["poll_q"] = text
        await db.users.update_one({"_id": user_id}, {"$set": {"state": "WAIT_POLL_OPT", "temp_data": temp_data}}, upsert=True)
        await clean_chat_history(user_id, chat_id, context)
    ...
    if state == "WAIT_POLL_OPT" and await has_perm(user_id, "publish"):
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إلغاء الاستجابة الافتراضية هنا لتجنب حجب التنبيهات المخصصة بالأسفل
    ...
    # الرد على استدعاءات الأزرار العادية
    try: await query.answer()
    except: pass
    ...
    if data.startswith("comp_les_"):
        ...
        if not all_qs: return await safe_edit(query, "⚠️ لا توجد أسئلة مضافة لهذا الدرس!", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 تراجع", callback_data=f"comp_cat_{str(oid)}")]]))
    ...
    if data.startswith("editadm_") and await has_perm(user_id, "manage_admins"):
        ...
        if not adm_doc: return await safe_edit(query, "⚠️ لم يتم العثور على المشرف.", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_manage")]]))
    ...
    if data.startswith("mgr_add_les_"):
        ...
        if not doc: return await safe_edit(query, "عنصر غير موجود.", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_content_mgr")]]))
    ...
    if data.startswith("mgr_les_"):
        ...
        if not oid: return await safe_edit(query, "⚠️ القائمة قديمة، يرجى تحديث النظام بإرسال /start.")
        doc = await db.library.find_one({"_id": oid})
        if not doc: return await safe_edit(query, "⚠️ الدرس غير موجود.", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_content_mgr")]]))
    ...
    if data == "admin_poll" and await has_perm(user_id, "publish"):
        ...
        await safe_edit(query, "📊 <b>إنشاء استفتاء للقناة:</b>\n\nأرسل الآن <b>سؤال الاستفتاء</b>:", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 تراجع", callback_data="admin_publishing_hub")]]))
        return
    ...
    if data.startswith("pubmed_"):
        ...
        if fmt_type == "btns":
            ...
            if not tpl_doc: return await safe_edit(query, "القالب غير موجود!", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="admin_tpl_menu")]]))
